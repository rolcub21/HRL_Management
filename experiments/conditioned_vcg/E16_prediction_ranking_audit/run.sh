...
case "${1:-}" in
  ...
  run-all)
    ...
    ;;
  *)
    echo "usage: $0 {analyze-offline|prepare-continuations|run-continuation-pilot|run-continuations|analyze-continuations|analyze-components|prepare-component-rollouts|run-component-pilot|run-component-confirmation|analyze-component-rollouts|run-all}" >&2
    exit 2
    ;;
esac
