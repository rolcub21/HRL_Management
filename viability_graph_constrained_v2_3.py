"""Policy/operator-consistent constrained VCG V2.3 core and runtime.
...
isolated: V2.3 always samples the induced nested Lagrangian policy.  The
...
V2_3_POLICY_VERSION = "four_group_induced_nested_stochastic_lagrangian_policy_gamma1_v2_3"
V2_3_DIAGNOSTIC_VERSION = "executed_outer_and_selected_within_distribution_v2_3"


@dataclass(frozen=True)
class ConstrainedV23Config(v2.ConstrainedV2Config):
    """V2.2 configuration schema with only the operational gamma unfrozen."""

    gamma_operational: float = 1.0

    def __post_init__(self) -> None:
        gamma = self.gamma_operational
        if isinstance(gamma, bool) or not math.isfinite(float(gamma)) or float(gamma) != 1.0:
            raise ValueError("V2.3 requires gamma_operational=1.0")
        # Reuse the complete audited parent validation for every other field,
        # substituting only the parent family's hard-frozen gamma value.
        parent_values = asdict(self)
        parent_values["gamma_operational"] = 0.99
        v2.ConstrainedV2Config(**parent_values)

    @classmethod
    def from_dict(cls, values: Mapping) -> "ConstrainedV23Config":
        if not isinstance(values, Mapping):
            raise TypeError("V2.3 configuration must be a mapping")
        expected = set(cls.__dataclass_fields__)
        observed = set(values)
        if observed != expected:
            raise ValueError(
                "V2.3 configuration field mismatch: "
...
def _validate_schedule_mapping(state: Mapping) -> dict:
    if not isinstance(state, Mapping):
        raise TypeError("V2.3 schedule state must be a mapping")
    received = dict(state)
    episode = received.get("episode_number")
    if isinstance(episode, bool) or not isinstance(episode, int):
        raise ValueError("V2.3 schedule requires integer episode_number")
    ...
    if context == "training":
        ...
        raise ValueError("V2.3 schedule execution_context is invalid")
    if received != expected:
        ...
        raise ValueError(f"V2.3 schedule does not match frozen clock: {differences!r}")
    return deepcopy(expected)


class ConstrainedV23HierarchyAgent(v2.ConstrainedV2HierarchyAgent):
    """V2 vector critic with independent behavior and replay RNG streams."""

    def __init__(self, *args, **kwargs) -> None:
        config = kwargs.get("config", args[0] if args else None)
        if not isinstance(config, ConstrainedV23Config):
            raise TypeError("V2.3 requires an explicit ConstrainedV23Config")
        expected = {
        ...
        mismatches = {
            key: (float(getattr(config, key)), value)
            for key, value in expected.items()
            if float(getattr(config, key)) != value
        }
        if mismatches:
            raise ValueError(f"V2.3 gamma-ablation config mismatch: {mismatches!r}")
        super().__init__(*args, **kwargs)
        self._v2_3_schedule_state: Optional[dict] = None
    ...
    def schedule_state(self) -> Mapping:
        return {} if self._v2_3_schedule_state is None else deepcopy(self._v2_3_schedule_state)

    def set_schedule_state(self, state: Mapping) -> None:
        self._v2_3_schedule_state = _validate_schedule_mapping(state)

    def _require_schedule(self) -> dict:
        if self._v2_3_schedule_state is None:
            raise RuntimeError("V2.3 policy schedule has not been installed")
        return self._v2_3_schedule_state
    ...
    def within_group_temperature_values(self) -> tuple[float, float, float, float]:
        ...
        if len(values) != 4:
            raise RuntimeError("V2.3 schedule lost a within-group temperature")
        return values
    ...
    def group_temperature(self) -> float:
        ...
        if not math.isfinite(value) or value <= 0.0:
            raise RuntimeError("V2.3 group temperature is invalid")
        return value

    def reset_behavior_rng(self, seed: int) -> None:
        if isinstance(seed, bool) or not isinstance(seed, int) or seed < 0:
            raise ValueError("V2.3 behavior RNG seed must be a non-negative integer")
    ...
    def _record_policy_diagnostic(self, decision) -> None:
        ...
        diagnostic = {
            "protocol": V2_3_DIAGNOSTIC_VERSION,
        ...
        if self.decision_log:
            self.decision_log[-1]["v2_3_policy_diagnostic"] = dict(diagnostic)
    ...
    def select(self, snapshot, *, training: bool = True, epsilon=None):
        """Sample V2.3's policy; ``training`` authorizes mutation elsewhere only."""

        schedule = self._require_schedule()
        expected_training = schedule["execution_context"] == "training"
        if bool(training) != expected_training:
            raise RuntimeError("V2.3 external training flag disagrees with execution context")
        if self.behavior_rng_seed is None:
            raise RuntimeError("V2.3 behavior RNG was not installed for this rollout")
        epsilon_value = self.epsilon if epsilon is None else self._validate_epsilon(epsilon)
        if epsilon_value != 0.0:
            raise RuntimeError("V2.3 epsilon invariant was violated")
        ...
        if not prepared.records:
            raise NoCertifiedViableAction("exact verifier exposed no SAFE V2.3 macro")
        operational, rehandle = self._score_records(prepared.records)
        elapsed = {record.elapsed_steps for record in prepared.records}
        if len(elapsed) != 1:
            raise ValueError("one V2.3 frontier contains mixed elapsed contexts")
    ...
    def checkpoint_metadata(self) -> dict:
        ...
        metadata.update({
            ...
            "policy_version": V2_3_POLICY_VERSION,
            ...
            "policy_diagnostic_version": V2_3_DIAGNOSTIC_VERSION,
    ...
    def load_checkpoint_state(self, state: Mapping, *, resumable: bool) -> None:
        if "schedule_state" not in state:
            raise ValueError("V2.3 agent state is missing schedule_state")
        ...
        self._v2_3_schedule_state = schedule
        if state.get("replay_rng_seed") != REPLAY_RNG_SEED:
            raise ValueError("V2.3 replay RNG seed mismatch")
        if resumable:
            for key in ("behavior_rng_state", "replay_rng_state"):
                if key not in state:
                    raise ValueError(f"resumable V2.3 state is missing {key}")
        ...
        if diagnostics:
            if diagnostics.get("protocol") != V2_3_DIAGNOSTIC_VERSION:
                raise ValueError("V2.3 policy diagnostic version mismatch")
    ...
    def from_checkpoint(cls, payload: Mapping, *, device="cpu", resumable=False, seed=10):
        ...
        if mismatches:
            raise ValueError(f"incompatible constrained V2.3 checkpoint: {mismatches!r}")
        agent = cls(
            config=ConstrainedV23Config.from_dict(payload["config"]),
        ...
        if dict(payload.get("schedule_state", {})) != dict(agent.schedule_state):
            raise ValueError("V2.3 checkpoint metadata/state schedule mismatch")
        return agent


class _FrozenV23EvaluationAgent(ConstrainedV23HierarchyAgent):
    def remember(self, *args, **kwargs):
        raise RuntimeError("V2.3 frozen validation clone forbids replay mutation")

    def learn(self, *args, **kwargs):
        raise RuntimeError("V2.3 frozen validation clone forbids learning")


class ConstrainedV23DevelopmentRuntime(v2.ConstrainedV2SmokeRuntime):
    ...
    def __init__(self, *, args, contract: Mapping) -> None:
        ...
        if mismatches:
            raise ValueError(f"V2.3 runtime contract mismatch: {mismatches!r}")
        if contract.get("dense_objective_spec") != FROZEN_OBJECTIVE_SPEC.to_dict():
            raise ValueError("V2.3 dense-objective contract mismatch")
        if dict(contract.get("viability_search_config", {})) != FROZEN_VIABILITY_SEARCH_CONFIG:
            raise ValueError("V2.3 viability-search contract mismatch")
        ...
        if hold_mismatches:
            raise ValueError(f"V2.3 Hold contract mismatch: {hold_mismatches!r}")
        ...
        self.config = ConstrainedV23Config.from_dict(contract["agent_config"])
        self.model_seed = int(contract["model_seed"])
        self.agent = ConstrainedV23HierarchyAgent(
        ...
        self._last_evaluation_agent: Optional[_FrozenV23EvaluationAgent] = None
    ...
    def begin_validation_batch(self) -> Mapping:
        if self._validation_batch_signature_before is not None:
            raise RuntimeError("V2.3 validation batch is already active")
        if self._schedule_state.get("execution_context") != "validation_deployment":
            raise RuntimeError("V2.3 validation batch requires validation schedule")
    ...
    def end_validation_batch(self) -> Mapping:
        ...
        if before is None:
            raise RuntimeError("V2.3 validation batch is not active")
    ...
    def _evaluation_agent(self, *, seed: int) -> _FrozenV23EvaluationAgent:
        del seed
        if self._pending_policy_rng_seed is None:
            raise RuntimeError("V2.3 validation action RNG was not installed")
        cpu_rng_state = torch.random.get_rng_state()
        cuda_rng_states = torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None
        try:
            clone = _FrozenV23EvaluationAgent(
                config=self.config, seed=self.model_seed, device=self.device, epsilon=0.0,
    ...
    def run_episode(self, *, instance_seed: int, training: bool, max_steps: int,
        ...
        expected_training = schedule["execution_context"] == "training"
        if bool(training) != expected_training:
            raise RuntimeError("V2.3 run flag disagrees with execution context")
        if isinstance(policy_rng_seed, bool) or not isinstance(policy_rng_seed, int) or policy_rng_seed < 0:
            raise ValueError("V2.3 requires an explicit action RNG seed per rollout")
        if isinstance(policy_rng_index, bool) or not isinstance(policy_rng_index, int) or policy_rng_index < 0:
            raise ValueError("V2.3 requires an explicit action RNG index per rollout")
        if 622_000_000 <= policy_rng_seed < 622_000_120:
            raise ValueError("V2.3 refuses the unopened final action-RNG panel")
        if 86_000 <= int(instance_seed) < 86_030:
            raise ValueError("V2.3 refuses the unopened final EpisodeInstance panel")
        if 83_000 <= int(instance_seed) < 84_000:
            raise ValueError("V2.3 refuses the protected prior prospective 83xxx panel")
        ...
        if training:
            ...
            if self._validation_batch_signature_before is None:
                raise RuntimeError("V2.3 validation rollout requires an authenticated batch")
        ...
        if episode_agent is None:
            raise RuntimeError("V2.3 validation clone was not retained")
        if result.get("episode_instance_id") != canonical_instance.instance_id or result.get("schedule_id") != canonical_instance.schedule_id:
            raise RuntimeError("V2.3 runtime EpisodeInstance identity mismatch")
        if not training:
            if self._last_evaluation_before is None:
                raise RuntimeError("V2.3 validation diagnostic baseline missing")
            before = self._last_evaluation_before
            if self._last_evaluation_selection_before is None:
                raise RuntimeError("V2.3 validation selection baseline missing")
...
def build_v2_3_development_runtime(*, args, contract: Mapping) -> ConstrainedV23DevelopmentRuntime:
    return ConstrainedV23DevelopmentRuntime(args=args, contract=contract)


__all__ = [
    "ConstrainedV23Config", "ConstrainedV23DevelopmentRuntime", "ConstrainedV23HierarchyAgent",
    "V2_3_DIAGNOSTIC_VERSION", "V2_3_POLICY_VERSION",
    "build_v2_3_development_runtime",
]
