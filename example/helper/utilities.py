...
class LinearSchedule:
    """
    Linearly anneal a value from `start` to `end` over the first
    (1 - end_fraction) of training, then hold it at `end`.
    """

    def __init__(self, start: float, end: float, end_fraction: float) -> None:
        assert 0.0 <= end_fraction <= 1.0, "end_fraction must be in [0,1]"
        self.start = start
        self.end = end
    ...
    def __call__(self, progress_remaining: float) -> float:
        """
        :param progress_remaining: fraction of training left in [0,1].
                                   1.0 at t=0, 0.0 at t=end.
        """
        # Clamp to [0,1]
        p = max(0.0, min(1.0, progress_remaining))

        # If we've passed the fraction threshold, return `end`
        if p <= (1.0 - self.end_fraction):
            return self.end

        # Otherwise linearly interpolate between start (at p=1.0)
        # and end (at p=1.0 - end_fraction)
        scale = (p - (1.0 - self.end_fraction)) / self.end_fraction
        return self.end + scale * (self.start - self.end)
